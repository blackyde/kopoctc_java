...
if (isMultipart) { // multipart로 전송 되었을 경우
	...
	b.setWrite_num(Integer.parseInt(param.get("write_num")));
	b = bb.selectOne(b);
	b.setSubject(param.get("subject"));
	b.setContents(param.get("contents"));
	List<String> oldfile = b.getFile();
	if(oldfile == null) {
		oldfile = new ArrayList<String>();
	}
	for(int i = 0; i < Integer.parseInt(param.get("totalFile")); i++) {
		String fileStr = "file" + i;
		if(param.get(fileStr) != null) {
			if(param.get(fileStr).equals("on")) {
				oldfile.remove(i);
			}
		}
	}
...
<form method="post" action="/Resort/" name="fr">
	<input type="hidden" name="contentPage" value="news/reboard/item.jsp">
